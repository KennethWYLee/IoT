...
課程前 90 分鐘全班同步操作，後 75 分鐘完成挑戰、保存證據及個別驗收。
完成全部驗收、斷電與收納後，可以先離開。
...
## 五、階段 2：Arduino IDE、Board 與 Port（約 10 分鐘）
...
完成下列所有項目。組員可以討論，但每位學生都必須實際操作並能回答。
...
不可使用會讓按鈕失去反應的 `delay(10000)`；使用 `millis()` 比較時間。
...
- [ ] 每位組員都能選擇正確 board 與 port 並上傳。
- [ ] Serial log 有組別、版本、按鈕事件、計數與 idle 訊息。
...
全部通過並簽核後，可以先離開。由一位組員包辦、缺少個人回答、未斷電或
未收納，都不算完成。

## 十四、個別口頭題目

1. Verify 成功與 Upload 成功有何不同？
2. 為什麼只有充電功能的 USB 線無法上傳？
3. `INPUT_PULLUP` 下，為什麼按下按鈕讀到 LOW？
4. 為什麼 5V 訊號不能直接進 ESP32 GPIO？
5. GND 在量測與訊號傳輸中有什麼用途？
6. 為什麼 GPIO 不能直接供電給馬達或舵機？
7. Upload 成功但 Serial 沒輸出，你會依序檢查什麼？
8. 通斷檔與直流電壓檔分別在什麼狀態使用？
9. 為什麼程式印出 GPIO5=HIGH 後，仍要用萬用電表驗證？
10. 哪一項證據能證明板子執行的是你的新版本，而不是舊程式？
