...
本教材把名詞放在實際會用到它的操作前面，不要求先背完一大張名詞表，也不
要求任何人一邊念、一邊接線。

1. 直接從「Part 1：逐步操作」依序進行；每張卡先解釋當下會遇到的名詞。
2. 先讀完目前操作卡的名詞、步驟及停止條件，再開始動手。
3. 名詞說明只放在第一次實際使用的位置；讀過後不用另外背誦名詞表。
...
- Part 1：從接上 USB 到按鈕、GPIO5 與萬用電表的完整實作；名詞會在使用前
  就地解釋。
- Part 2：必要修改、九種變化題、故障排查、繳交與驗收。
...
每張卡只解釋現在要用的名詞。先把當張卡的「名詞說明」和步驟完整讀完，再
動手，最後檢查「通過條件」。如果沒有通過，就留在該卡排錯。
...
5. 確認按鈕顯示 `REMOVE` 或旁邊標示已安裝版本；若仍顯示 `INSTALL`，表示
   課前作業尚未完成。`INSTALL` 是安裝，`REMOVE` 是移除；看到 `REMOVE`
   時不要按下它。
6. 把已安裝版本填入：
...
- 這裡的 **COM5** 是 Windows 序列連接埠名稱；後面萬用電表上的 **COM**
  是黑表筆插孔，兩者完全不同。
...
- `unsigned long` 是可保存非負整數的資料型別，這裡用來保存毫秒時間。
- `millis()` 取得 ESP32 從開機到現在經過的毫秒數。
...
而是本教材自行貼的標籤：TP5 之後接 GPIO5，TPG 之後接 GND。
...
**電壓**是兩點之間的電位差，不是單獨一點自帶的數字。這裡量的是 TP5 相對
...
這裡新增的 **變數**是有名稱、可保存及改變的資料。`pressCount` 使用
