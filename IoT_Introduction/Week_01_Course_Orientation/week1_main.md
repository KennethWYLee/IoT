...
6. Week 15第二次專題報告可以檢查的最低成果。

完整成品不是Week 8要求。題目過大時，應刪減功能，保留可以測試的核心流程。

### 第二次個人筆試（Week 13）：網路通訊與軟硬整合架構

筆試檢查Week 6及Week 10～12的整合概念，包括Wi-Fi、IP、HTTP、JSON、
WebSocket、MQTT、Backend、Database、手機前台，以及sensor signal、event、
command、result與log如何跨層流動。全週只進行個人作答，不安排新進度或實作。

### 第二次專題報告（Week 15）：端到端整合、使用性與修正

先展示真實硬體到Backend、Database與手機的端到端資料路徑，再由非開發者使用
手機完成核心流程三次，依操作問題或log修正作品並提出修正前後證據。
...
替換零件。完整使用週次與到貨檢查見[Week 1支援資料](week1_support.md)。
