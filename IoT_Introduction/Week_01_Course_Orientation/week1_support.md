...
OLED價格與新版總額待確認，另計USB線、電池、安全端子、組內電表、必要固定材料與運費。
...
| 紙盤、短紙指針、底板與固定材料 | 現有輕量紙材；不需承重支架，不黏住軸 | 1套 | Week 6 |
...
訂單圖2中，本課必買的是母對母杜邦線。L298N、MG90S、HC-SR04與萬向球不屬於共同必買。
OLED已列新共同需求，但本張購買圖不等於核准型號。
...
共同遊戲的功能關係如下。這是教學示意圖，不是已完成實機驗證的成品，也不是接線圖；
圖中箭頭表示資訊或控制關係，不能當成實體電線。Week 1只辨認用途，不操作硬體。
按下開始後，綠燈允許遮光、紅燈遮光扣次數；達到6次仍須在期限前按結束。
OLED顯示倒數與結果，SG90帶動短紙指針指向有效次數；失敗只作有限時間蜂鳴。
後續會分週確認零件相容性，再逐步整合，不要求第一週組出成品。

![紅綠燈遮光挑戰功能示意：感測、按鈕、規則與顯示輸出；非實物接線圖](../../docs/images/wiring/week7-system.png)

| 週次 | 當週硬體準備 |
|---:|---|
| 1 | 不帶硬體、不接線；課後依核准規格準備 |
| 2 | ESP32-S3、USB、麵包板、公對公／公對母線、第一顆按鈕、組內電表 |
| 3 | KY-018、1kΩ／10kΩ、原板與電表；新增分類沿用接線 |
| 4 | 220Ω／330Ω、DHT11、KY-018、三種線材、蜂鳴器與組內電表 |
| 5 | RGB、核准OLED、原板與線材；光敏用於干擾觀察；不接外部舵機電源 |
| 6 | SG90、紙盤短指針、固定材料、核准4AA電源／電池／端子、電表 |
| 7 | 重用Week 2～6材料與兩顆按鈕，完成遊戲；不用網路 |
| 8 | 攜帶可運作硬體片段與材料／風險計畫 |
| 9 | 不要求到校、不增加必做成果 |
| 10 | 個人筆試，不操作硬體 |
| 11 | 板卡、按鈕、低功率RGB、筆電與網路；不接SG90、蜂鳴器、4AA |
| 12 | 板卡、核准輸入／低功率輸出、broker、後端與資料庫 |
| 13 | 攜帶目前專題實作版本與進度證據 |
| 14 | 依核准專題使用基本包；手機前台與權限 |
| 15 | 依核准專題使用基本包；自動反應、故障復原、重建 |
| 16 | 個人筆試，不操作硬體 |
| 17 | 完整專題與備援、展示與個人問答；每組只一次 |
| 18 | 保留空白，不要求器材或新成果 |
