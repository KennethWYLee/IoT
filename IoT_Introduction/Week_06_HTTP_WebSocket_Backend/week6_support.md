...
| 項目 | 檢查方法 | 實際結果 | 可開始條件 |
|---|---|---|---|
| 本機repository | `git status`、`git pull --ff-only` |  | 無未處理衝突；可看到Week 6與Backend |
| Python | PowerShell執行`python --version` |  | 可啟動且版本已記錄 |
| Git | PowerShell執行`git --version` |  | 可執行Git命令 |
| Arduino IDE | 開啟IDE並讀取About／版本畫面 |  | Week 5相同環境可用 |
| esp32 board package | Boards Manager讀取已安裝版本 |  | Week 5相同版本可用 |
...
| 品項 | 數量 | 本週用途 | 上電前確認 |
|---|---:|---|---|
| ESP32-S3-DevKitC-1 | 1 | Wi-Fi裝置與GPIO控制 | 型號、USB端口、排針無彎折 |
| USB資料線 | 1 | Upload及USB供電 | Week 5已驗證可傳資料 |
| 400孔麵包板 | 1 | START、STOP與RGB接線 | 電源軌方向已辨認 |
| 四腳按鈕 | 2 | START與本機STOP | Week 5腳位方向已辨認 |
| KY-016 RGB模組 | 1 | IDLE／ACTIVE／ERROR輸出 | Week 4 ON level已記錄 |
...
| START按鈕無事件 | hardware input | 拔USB後核對INPUT_PULLUP與GND | Week 5接法一致 | 不先改網路 |
| RGB顏色相反 | hardware profile | 拔USB後核對ON level | Week 4 profile一致 | 不隨機交換GPIO |
...
- [Week 5單機互動主教材](../Week_05_Standalone_Interaction/week5_main.md)
