...
| KY018-T01 | KY-018相容光敏電阻模組 | 已取得1個實物；其餘待盤點 | unverified | [元件面](images/hardware/actual/ky018-photoresistor-module-actual-component-side.jpg)、[焊接面](images/hardware/actual/ky018-photoresistor-module-actual-solder-side.jpg)與[排針近照](images/hardware/actual/ky018-photoresistor-module-actual-pin-labels.jpg)；排針朝下時可見左側獨立`S`、右側獨立`-`；中央可讀到`A`／`S1`／`R1`，但它們位於元件焊點區，不能當作中間排針標示；只從照片可確認`R1`是固定電阻的常見元件編號，未判定`A`與`S1`用途 | 保持斷電；以電阻量測確認`S`、`-`與未標示中間腳的分壓關係，再決定3V3、GND與ADC接線；尚未通電 |
