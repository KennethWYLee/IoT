...
新完整設計以`docs/course_materials/traffic_light_challenge_design.md`為依據。
階段末用量剩餘26%；進入實作前25%。
...
期末展示稿保存在docs/archive，不直接刪除內容。Week 12合併MQTT及資料庫完整步驟，
...
- 後端／資料庫／MQTT bridge：既有`examples/course_backend/.venv`中執行pytest，26 passed。
...
- `docs/course/`、`docs/course_materials/`及`docs/hardware/`：課綱、課卡、設計標準、
  遊戲規則權威文件、Notebook維護來源、材料用途及驗證限制。
- `examples/week03_light_classifier/`至本次新增的`examples/week07_traffic_light_challenge/`：
  六支新增完整程式；既有Week 2程式未變動。
- `docs/images/wiring/`：27組新SVG／PNG概念圖；`docs/images/hardware/actual/`補存教師原始電阻照片。
- `scripts/`：生成一致性、Notebook呈現、Arduino編譯及實際sketch的host stub檢查。
- Week 8～17目錄、README導覽與`docs/archive/`：依核准課綱搬移、合併及封存；不取消個人問答。
