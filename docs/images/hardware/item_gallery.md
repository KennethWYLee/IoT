...
排針近照在排針朝下時可見左下角獨立`S`與右下角獨立`-`。中央的`A`、`S1`
與`R1`位於元件區；`R1`是固定電阻的元件編號，不能當作中間排針標示。中間腳
仍須由技術來源及斷電電阻量測確認後才能通電。

![KY-018光敏電阻模組排針與元件絲印近照](actual/ky018-photoresistor-module-actual-pin-labels.jpg)
