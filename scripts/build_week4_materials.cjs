...
// Canonical prose: week4_main.source.md; code: examples/week04_*/*.ino.
...
const sourcePath=path.join(root,'docs/course_materials/week4_main.source.md');
...
async function build(){
 ...
 for(const section of source.split(/<!-- cell -->/)){
   ...
   for(let i=0;i<pieces.length;i++)if(i%2===0)markdown(pieces[i]);else{
     const name=pieces[i],code=fs.readFileSync(path.join(root,`examples/${name}/${name}.ino`),'utf8').replace(/\r\n/g,'\n');
 ...
 const nb={cells,metadata:{language_info:{name:'cpp'},course_edition:'complete-preparation-with-reference-answers',maintenance_source:'docs/course_materials/week4_main.source.md',hardware_validation:'pending-exact-module-and-gpio-profiles'},nbformat:4,nbformat_minor:5};
