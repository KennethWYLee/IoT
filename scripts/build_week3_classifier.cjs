...
const sourcePath=path.join(root,'docs/course_materials/week3_classification.source.md');
...
async function main(){
 for(const [name,markup]of Object.entries(figures())){const file=path.join(root,`docs/images/wiring/week3-classifier-${name}`);artifact(file+'.svg',markup);artifact(file+'.png',await sharp(Buffer.from(markup)).png().toBuffer());}
