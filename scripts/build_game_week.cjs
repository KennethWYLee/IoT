...
assert(directories[week],'Usage: node scripts/build_game_week.cjs 5|6|7 [--check]');
const sourcePath=path.join(root,`docs/course_materials/week${week}_main.source.md`);
...
async function main(){
 ...
 for(const [name,svg]of Object.entries(figures)){
  const base=path.join(root,`docs/images/wiring/week${week}-${name}`);
 ...
 for(const section of source.split(/<!-- cell -->/)){
  ...
  pieces.forEach((piece,i)=>{if(i%2===0)markdown(piece);else{const code=fs.readFileSync(path.join(root,`examples/${piece}/${piece}.ino`),'utf8').replace(/\r\n/g,'\n');cells.push({cell_type:'code',metadata:{language:'cpp',sketch:piece},execution_count:null,outputs:[],source:code.match(/.*\n|.+$/g)});}});
 }
 cells.forEach((c,i)=>c.id=`w${week}-`+crypto.createHash('sha256').update(i+':'+c.source.join('')).digest('hex').slice(0,12));
 artifact(notebookPath,JSON.stringify({cells,metadata:{language_info:{name:'cpp'},course_edition:'complete-preparation-with-reference-answers',maintenance_source:`docs/course_materials/week${week}_main.source.md`,hardware_validation:'pending-exact-module-and-gpio-profiles'},nbformat:4,nbformat_minor:5},null,1)+'\n');
