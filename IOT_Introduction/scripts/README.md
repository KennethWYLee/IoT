...
```powershell
node IOT_Introduction/scripts/export_notebook_pdfs.cjs
node IOT_Introduction/scripts/export_notebook_pdfs.cjs --check
```

Export requires Node.js, `marked`, `playwright`, Microsoft Edge, and Microsoft
JhengHei (or a reviewed Traditional Chinese font). Set `NODE_PATH` when using
bundled packages; `BROWSER_CHANNEL` can select an installed Chromium browser.
The exporter embeds local images, preserves section links, and points other local
document links to GitHub. Remote image requests and unsupported saved outputs stop
the export rather than silently dropping content. No notebook execution occurs.
The read-only `--check` needs only Node.js and Git. It verifies the complete notebook
inventory and SHA-256 records for notebooks, image dependencies, exporter and PDFs
in `docs/notebook_pdf_manifest.json`. Text hashes normalize CRLF/LF for cross-computer
use. Regenerate after source/image/exporter changes, inspect the PDF pages, and commit
the PDFs and manifest together. HTML previews are ignored under `_outputs/notebook_pdfs/`.
Hash checks establish freshness, not visual quality or physical-device correctness.
...
the 18 weekly directory structure (notebooks plus same-name PDFs in Weeks 2–7,
one Markdown file elsewhere), blank Week 18, Chinese unit overview headings and content,
