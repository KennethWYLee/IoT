...
for(const [i,name]of ['week04_dht11_quality','week04_dual_sensor_alarm'].entries()){
 assert.equal(code[i].source.join(''),fs.readFileSync(path.join(root,`IOT_Introduction/examples/${name}/${name}.ino`),'utf8').replace(/\r\n/g,'\n'));
...
for(const c of nb.cells){
 ...
 for(const m of s.matchAll(/!\[[^\]]+\]\(([^)]+)\)/g)){
   ...
   const photo=Object.values(gallery.catalog.components).flatMap(p=>p.photos).find(p=>path.basename(p.file)===key);
   const original=key.startsWith('week4-')?path.join(root,'IOT_Introduction/docs/images/wiring',key):path.join(gallery.base,photo.file);
...
assert.equal(images,16+require('./hardware_galleries.cjs').imageCount(4));assert(!all.includes('<!-- sketch:'));
...
console.log(`PASS ${nb.cells.length} cells, ${images} verified images (byte-matched attachments plus hashed shared photos), ${links} local links/anchors, two source-identical gated sketches, seven Discussion questions, example math and modeled breadboard nodes.`);
...
async function render(){
 ...
 let body='';nb.cells.forEach((c,i)=>{let s=require('./hardware_galleries.cjs').inlineLocalPhotos(c.source.join(''),file);for(const [name,p]of Object.entries(c.attachments||{})){const [mime,data]=Object.entries(p)[0];s=s.replaceAll('attachment:'+name,`data:${mime};base64,${data}`);}body+=`<section id="cell-${i}">${c.cell_type==='code'?'<pre><code>'+esc(s)+'</code></pre>':marked.parse(s)}</section>`;});
 ...
 try{
  ...
  const figureDir=path.join(root,'IOT_Introduction/docs/images/wiring');let count=0;
  ...
  console.log(`PASS Edge notebook-style preview: ${images} decoded images, 1200/420px no page overflow; ${count} SVG text-boundary checks. Manual image review is separate.`);
