...
> `IOT_Introduction/docs/hardware/hardware_state.md`公布Week 4 hardware profile前，不得自行填值、接上
> 電池或讓舵機動作。
...
| 萬用電表 | 每組1台；1人組可跨組共用 | 極性、空載及負載電壓；各組分別保存量測證據 |
...
開始操作前，將`IOT_Introduction/docs/hardware/hardware_state.md`中已完成target test的數值抄入
