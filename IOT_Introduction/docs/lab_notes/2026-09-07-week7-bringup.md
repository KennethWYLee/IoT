...
- 電池實際為四顆1.5 V AA，回報6.6 V；當時教師沿用既有降壓模組作補充測試。後續使用者已核准學生採相同供電架構、供電組可按組共買，取代原4×1.2 V鎳氫方案；採購與預算以Week 1為準。此決策不新增舵機實測證據。
...
上節是原實作整理的檢查紀錄。其後使用者核准課程使用今天的供電方式，價格較高的供電組可按組購買。
當時方案改為四顆1.5 V AA、帶開關電池盒與可調降壓模組；供電組可由1～3人輪流使用，
一次只供一顆SG90，換接先關電池並拔USB。原鎳氫電池／充電器安排不再適用；當時其他個人零件數量不變，後續全套按組決策見下節。
採購規格與重算預算唯一維護於[Week 1](../../Week_01_Course_Orientation/week1_main.md#purchase-table)。

- 更新Week 1採購／預算、Week 6／7維護來源及Notebook、相應材料安排／設計／硬體狀態，並把降壓模組原圖加入Week 1／6／7圖集。
- 文件：88份文件與本機連結檢查通過；11週圖集及44筆照片來源雜湊通過。
- 生成：Week 6／7重建後`--check`通過；兩份Notebook程式與未改動的canonical來源一致。
- 呈現：本機Edge桌面1200px／手機420px通過；Week 6／7分別31／36張圖片解碼及15張既有SVG文字邊界通過。人工查看Week 1供電表／預算及Week 6降壓章節，未見遮擋；未測即時GitHub。
- 計價：個人表格逐列加總與1／2／3人組分攤算式通過；歷史價格不當作即時報價，電池及連接材料仍另計。
- 程式編譯、host、simulation、Upload及實機：此採購調整未重新執行，不新增測試通過主張；舵機負載仍待驗。
- Git：差異與`git diff --check`通過；Week 2／3與範例程式無內容變更。此次修改尚未commit／push，PROJECT仍僅留本機。

## 最新採購單位：全套由每人改為每組

使用者再核准「每人改每組」：每組1～3人共用一套基本零件、舵機供電組、電表、USB資料線與收納用品，
不再只限供電組共用。個人筆電、輪流實際操作及各自學習紀錄保留；品項規格與單套數量不變。
每組總額及每人分攤以Week 1為唯一現行預算，上節個人材料估算不再適用。

同步改Week 1、Week 2器材表五列、Week 11器材導言、中文課綱、採購原則、材料runbook及庫存的學生需求摘要；
不更動教師歷史購買數量、程式、圖片或評量。文件檢查器的標題要求同步改為每組。
重新核對Week 1逐列加總與分攤，檢視桌面／手機版面及圖片；Week 2文件／原程式一致性與JS去抖模型、
11週圖集／44筆圖片雜湊檢查通過。文件連結檢查在更新舊「每人」標題斷言後重跑；
無新增編譯、Upload或實機驗證。修改保留本機，尚未commit／push。
