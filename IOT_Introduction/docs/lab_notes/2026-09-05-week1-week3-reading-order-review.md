...
- `IOT_Introduction/scripts/verify_intro_navigation.cjs`：新增四份文件的錨點與閱讀順序檢查、Week 1本機渲染。
- `IOT_Introduction/scripts/verify_week2_notebook.cjs`、`IOT_Introduction/scripts/verify_week3_notebook.cjs`：配合新位置加入回歸斷言。
- `IOT_Introduction/scripts/build_week2_figures.cjs`：章節尋找允許標題前有錨點；沒有更動畫圖函式或產生新圖。
- `IOT_Introduction/scripts/README.md`：新增導航檢查的執行與限制說明。
