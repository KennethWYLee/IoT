...
parser = CodeBlocks()
parser.feed((HERE / "week5_main.html").read_text(encoding="utf-8"))
expected = "\n".join((COURSE / item["path"]).read_text(encoding="utf-8").rstrip()
                     for item in manifest["sketches"])
assert "\n".join(parser.blocks) == expected

doc = fitz.open(HERE / "week5_main.pdf")
...
assert "28500" in doc[ids["answer"] - 1].get_text()
assert "27.9" in doc[ids["answer"] - 1].get_text()
assert "225" in doc[ids["answer"] - 1].get_text()
assert 11500 - 10000 == 1500
assert 30000 - (11500 - 10000) == 28500
assert math.isclose(30 - .8 - 1.3, 27.9)
assert math.isclose((3.3 - 2.0) / 330 * 1000, 3.9393939393939394)
assert 128 * 64 // 8 == 1024
# Compare the published integer expression with mathematical ceiling.
for remaining in [0, 1, 999, 1000, 1001, 1499, 28500, 30000]:
    assert (remaining + 999) // 1000 == math.ceil(remaining / 1000)
assert max(0, 30000 - 31200) == 0
assert 31200 - 30000 == 1200

def node(hole):
    return ("left" if hole[0] in "abcde" else "right", int(hole[1:]))

assert node("a3") == node("b3") == node("c3") == node("e3")
assert node("a6") == node("b6") == node("c6") != node("a3")
assert node("a15") == node("c15") != node("f15")
assert len(set([15, 16, 17, 8, 9, 4])) == 6

# Check time boundaries independently of the program's rendering functions.
schedule = [(0, "GREEN"), (2999, "GREEN"), (3000, "RED"),
            (5999, "RED"), (6000, "GREEN"), (30000, "OFF")]
for elapsed, expected in schedule:
    label = "OFF" if elapsed >= 30000 else ["GREEN", "RED"][(elapsed // 3000) % 2]
