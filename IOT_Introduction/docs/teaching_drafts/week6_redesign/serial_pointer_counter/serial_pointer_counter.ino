// Extension of the course pointer: Serial + / - changes count.
// Physical STOP, timeout and all original power/profile guards are retained.
...
void handleCommand(char c,uint32_t now) {
  ...
  if(c=='+' || c=='-') {
    if(count==6 && c=='+') {
      Serial.println("event_type=command_rejected reason=upper_limit");return;
    }
    if(count==0 && c=='-') {
      Serial.println("event_type=command_rejected reason=lower_limit");return;
    }
