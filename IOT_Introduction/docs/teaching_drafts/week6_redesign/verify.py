...
assert sha(HERE / "week6_main.md") == manifest["sourceSha256"]
assert sha(HERE / "build.cjs") == manifest["builderSha256"]
assert sha(HERE / "week6_main.pdf") == manifest["pdfSha256"]
...
parser = CodeBlocks()
parser.feed((HERE / "week6_main.html").read_text(encoding="utf-8"))
expected = "\n".join((COURSE / item["path"]).read_text(encoding="utf-8").rstrip()
                     for item in manifest["sketches"])
assert "\n".join(parser.blocks) == expected

doc = fitz.open(HERE / "week6_main.pdf")
...
assert "31000" in doc[ids["answer"] - 1].get_text()
assert "5、6、4" in doc[ids["buildanswer"] - 1].get_text()
assert 60 + (120 - 60) * 4 // 6 == 100
assert 1000 + 30000 == 31000
n = 0
observed = []
for command in "++++----":
    n = min(6,n+2) if command=="+" else max(0,n-2)
    observed.append(n)
assert observed == [2,4,6,6,4,2,0,0]
n = 3
observed = []
for command in "++-":
    n = min(6,n+2) if command=="+" else max(0,n-2)
    observed.append(n)
assert observed == [5,6,4]
assert "int next =" not in doc[ids["buildexercise"] - 1].get_text()
assert len({7,10,8,9}) == 4  # Host fixture, not approved physical pins.
