...
# The added combined record defines its own prefixed age field; the original
# two-line sensor log must still use the canonical age_ms spelling.
original_text = "\n".join(doc[p["number"] - 1].get_text() for p in manifest["pages"]
                          if not p["id"].startswith(("combine", "build", "button_")))
assert "dht_age_ms" not in original_text
assert "dht_age_ms" in doc[ids["combinetry"] - 1].get_text()
