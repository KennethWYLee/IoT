...
**先學會重開，再開始計時：**每局結束後，兩鍵放開、手移開光敏，在 Monitor 上方輸入框打 `z`，按 Enter。看到 IDLE、count=0 才按 Start。若是 ABORTED，先排除 reason 指出的原因，送 c，再送 z。
...
上一局先讓它到期。看完本頁，再兩鍵放開、手移開、送 z，看到 IDLE 才按 Start，開新的一局。不要在 30 秒倒數中翻頁聽完全部解釋。
...
熟悉後可另開一局，在紅燈做一次新的遮光，應扣 1，但最低 0。若一直遮住，從紅變綠也不自動補加分。每局只練本頁一件事，到期就用 z 重開。
...
看完本頁再送 z 回 IDLE、按 Start。這一局先不遮光，隨即按 Finish，觀察 count=0 的結算；這是規則測試，不是操作做錯。
