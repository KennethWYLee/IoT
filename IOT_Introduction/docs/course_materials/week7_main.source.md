...
接下來的推理對應第8節同一支完整程式，不需先重寫一套遊戲。
先讀懂本節「一次事件」，再依第8.2節完成最小的一局；同輪期限與Finish的邊界，
回到第4節及Discussion用示例推理，不要求第一次操作便用手做出毫秒級邊界。
...
| 蜂鳴器（階段2） | PIN_BUZZER→Week 4核准接法；HW-508波形路徑必須經串聯1 kΩ | 選定有效準位或波形設定；不是任意裸蜂鳴器 |
...
下方正式遊戲已提供這兩個選項，預設仍是直接開／關與SSD1306。
...
依Week 5設定`OLED_CONTROLLER=1306`或1315，位址另填；都只適用核准的128×64 I2C配置。
Stage 1仍需ESP32Servo編譯，但不啟動舵機與蜂鳴器，也不配置蜂鳴器LEDC通道。

階段2沿用Week 4的蜂鳴器核准設定：直接開／關選`BUZZER_USE_TONE=false`並填有效準位；
HW-508受限波形選true，實物保留串聯1 kΩ並填`BUZZER_SERIES_OHMS=1000`，不使用ON_LEVEL。
兩種模式都只在FAILED後要求200 ms短聲。初始化失敗時停在blocked；發聲API失敗改為ABORTED，
停止API失敗也鎖住後續操作。這類故障不可用`c`／`z`清除，斷電檢查後重新啟動。
聲音與舵機都使用PWM資源；編譯及主機替身測試不代表兩者在實物上已同時正常。
完整階段2仍待供電、聲長、舵機動作與停止反應的整合實測，不跳過Week 6的核准條件。
...
先做第10.1、10.2節題目：預測一個新事件與紅扣分；其餘題目在基本遊戲有反應後接續。
再開sketch，
...
**保留什麼、新增什麼：** 沿用Week 3的KY位置與同配置基準、Week 5的RGB與OLED設定。
Week 2按鈕的電氣原理沿用，但本週加入第二顆按鈕並重新核對兩個輸入腳；
不能把Week 2的測試輸出線當成Finish輸入。Week 6的供電組與舵機先完全分離，DHT本週不接。
現物若仍與已保存配置相同，引用其紀錄後逐線核對，不要求把已完成的辨識實驗全部重做。

以下使用同一套完整規則，只逐段增加觀察焦點，**不是關掉紅燈或改變成功條件**：

| 觀察段 | 先預測 | 同一局要確認的結果 | 結果不同先查 |
|---|---|---|---|
| A：開始與時間 | 按Start後什麼先變？ | 新game_id、RUNNING、倒數開始、實物綠 | reason、按鈕狀態及第7.2命令前提 |
| B：一次事件 | 綠燈遮住不放一秒會加幾次？ | 加一次；放開只是準備下一次 | raw、settled、armed，不按log行數算分 |
| C：規則與結算 | 紅扣一次後不足6按Finish？ | 扣到下限0為止，Finish得到FAILED、時間凍結 | 最後cover與result，不拿舊的6截圖判成功 |

第一次低功率局可連續完成A～C。其餘成功、到期與復原證據依第11節矩陣補足；
同一段log能覆蓋的項目只記一次，不能把尚未觀察的項目打勾。
...
本段只增加已分別驗證的機械與聲音輸出，保留低功率階段的遊戲規則、光線位置、
校正及計時設定。先核對「count相同時各輸出是否一致」，不是重新排查所有感測器。
