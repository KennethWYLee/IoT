...
| [5～7：DHT11](#w4-dht) | 為何DATA不是另一個類比電壓測點？ | 三線接法、真實溫濕度log與欄位解釋 |
| [8～9：品質與恢復](#w4-quality) | 讀取失敗、超範圍與突變，有什麼不同？ | 真實→軟體注入→真實的分開紀錄 |
| [10～12：KY-018事件](#w4-sampling) | 原有數字如何變成一次穩定動作？ | 沿用基準、候選／穩定／事件預測 |
...
每組使用一套ESP32-S3、USB資料線、400孔麵包板、KY-018、YS-31 DHT11，
...
教師先完成同批規格、驅動與電流等核准並公布設定；學生對照自己實物、追線及記錄反應。
若實物與公布版本不同，保持斷電提出差異，不要求初學者自己猜額定值或勾true才能繼續。
...
DHT11-T01只有外觀照片，尚不足以公布實物上電接法。第5～6節會把確認項目與
...
以下僅適用第5節**全部核准且可用3.3 V**的模組。此階段先單獨測DHT11，
...
3. DHT11先不連任何線，依第7節安裝library、設定核准GPIO並Upload新程式。
...
以第5節核准表填`PIN_DHT`，確認表內所有條件後才將`MODULE_PROFILE_CONFIRMED`改為`true`；
...
| `reportReading()` | 依順序檢查並把值與品質一起印出 | 第8節說明先後順序；不是看到第一個數字就成功 |
...
精確規格由第5節模組文件與驗證紀錄確認。數值通過規則不能替代校正或精度驗證。
...
### 7.6 Verify、Upload與第一段真實紀錄

1. 依6.2保持DHT11未接、只連ESP32進行Upload。IDE `File → Open`開本節`.ino`，
   核准後填設定、另存自己的同名sketch資料夾。未核准只Verify，不繼續硬體步驟。
2. 按Week 2已確認的Board／Flash／PSRAM／Port設定核對，點Verify。
   `DHT.h: No such file`先查7.3library，不改接線、不將錯誤畫面當Upload成功。
3. 關Monitor、選實際Port、Upload，保存編譯與寫入結果。這次程式取代上一支KY程式，
   不是讓兩支同時在板上跑。寫入完成後斷電，依6.3接DHT11並通過6.4檢查。
...
第5～9節已完成DHT11單獨讀取、品質判斷與恢復；第10～11節接回KY-018的分類與穩定事件。
接下來到第12A節合併兩種感測器，最後加入已核准的蜂鳴器（buzzer）控制。
已通過的單一模組測試保留紀錄，不從電阻測試重新做起。

1. 保存DHT11程式、log及Week 3的KY-018基準紀錄，再拔除USB與其他電源。
2. 第12A節依新的同時使用接線表加入KY-018；逐一確認供電與兩條訊號路徑，不沿用舊接線猜腳位。
3. 蜂鳴器供電、控制方式、安靜時準位、電流及上電／Reset安全尚未核准時，
   可先完成兩個感測器已核准的階段1、2；只有聲音階段停在文件與Verify。
...
同一份完整程式以`LESSON_STAGE`選擇功能；每換階段保存程式與log，再依斷電步驟重查接線。

| 階段 | 現在接什麼 | 本次新增概念 | 完成證據 |
|---|---|---|---|
| 1：雙讀值 | KY-018＋DHT11，不接蜂鳴器 | 兩個來源各自取樣 | 兩種sensor紀錄、sample與age；KY狀態UNKNOWN、事件0 |
| 2：遮光事件 | 維持階段1接線 | 套用自己的基準並確認一次新動作 | 解除→遮住→保持→解除；保持時不加事件，buzzer_command=disabled |
| 3：聲音回饋 | 斷電加入已核准蜂鳴器 | 把新事件轉為有限短聲 | 一事件一短聲，q靜音，u不補播 |

階段1只填兩個不同GPIO與`SENSOR_PROFILES_CONFIRMED`；四個校正常數可先保持−1。
階段2再填同一配置的四個基準範圍。這兩階段的蜂鳴器GPIO保持−1、旗標保持false，程式不操作它。
公開預設為階段3且所有安全閘門未核准；第一次操作須明確改成1，不能把所有旗標一起改true。
...
第6節原DHT供電在c6／c3，**現在移到b6／b3，空出的c6／c3給KY**；
...
階段1、2只接表中的感測器路徑，略過兩列蜂鳴器接線；階段3才加上它。
兩個感測器GPIO必須不同；階段3的第三個GPIO也不能重複，程式會阻擋衝突。麵包板第3列不是GPIO3，
...
1. 先保存DHT單獨實驗的程式與紀錄。開上方完整整合程式，設LESSON_STAGE=1，
   只填已核准的感測器設定及不含姓名的DEVICE_ID。蜂鳴器完全不接。
2. 外部零件先不接ESP32，用Week 2同一套Board／Port、115200設定Verify及Upload。
   寫入完成後拔USB，依12A.3表只接感測器，再逐線核對、上電。
3. 確認開場lesson_stage=1、buzzer_enabled=false及status=ready。
   約500 ms起看到KY摘要；DHT須等約2500 ms，第一筆前snapshot=none不是0°C。
4. 固定位置，觀察一般光→遮光→移開。保存兩個來源至少各兩筆新採樣紀錄；
   不另重收校正用的20筆。light_raw應有可重現的方向變化，溫濕度不必隨遮光立刻變。
5. 找一筆DHT的新sample，再看後續snapshot的age增加；重印同一筆不算新採樣。
   KY的state=UNKNOWN、candidate=-1、armed=false、event_count=0是本階段刻意不分類的結果。

本階段KY的valid只排除缺值和ADC端點，未檢查校正範圍；DHT仍各自檢查有效性。
若DHT讀不到，先保存reason，拔USB後核對該模組接線；不要改動已有反應的KY線路。
完成條件是能指出兩種來源、各自更新時間與一項不能下的結論；有兩排數字不等於兩者都可信。
...
### 12A.5A 階段2：只確認事件，先不發聲

保持同一組感測器接線及位置，將LESSON_STAGE改為2，填入自己同一配置的四個基準。
Verify、Upload後開115200 Monitor；不新增蜂鳴器接線，也不改蜂鳴器旗標。
先保持未遮約一秒，等armed=true；遮住一秒，應出現一筆cover_event，
再保持一秒事件數不變；移開至少300 ms，再遮才可新增事件。
log的buzzer_command=disabled表示沒有要求聲音，不是蜂鳴器故障。
若事件連續增加，先保存candidate／state／armed，檢查分類與接觸；需動線時先拔USB。
完成這段「解除、一次遮光、保持、再次解除」的紀錄後，才新增下一段聲音回饋。

### 12A.5B 先辨認蜂鳴器，不能只看購物品名
...
完整接法、三支診斷程式與實際紀錄見[BOARD-T01實測交接](../lab_notes/2026-09-07-week7-bringup.md)。
...
### 12A.6 階段3：加入短聲，其他線與參數保持不變

1. 本段只在聲音profile核准後執行；否則保存階段1、2成果。將`LESSON_STAGE=3`，
   保留兩個感測器GPIO及校正值，再填蜂鳴器GPIO與核准旗標；
   依12A.5D選發聲方式：開／關模式填HIGH或LOW，波形模式核對串聯1 kΩ並填阻值確認。
   使用第7節相同DHT library及ESP32編譯設定，保存自己的整合版本。
...
事件發生後armed=false，直到重新穩定解除才變true。以第11節時間圖指出重新準備點。
...
參考第4節量程圖與第7節類比／數位圖。缺電表型號、檔位或單位時，C也只能待確認。

### 13.2 第二題：有波動就壞了嗎？

**已知／問題：** KY在固定室內光取10筆，依10.1資料300～320，平均310；
...
先查有限值，再RH，再政策，最後突變；第8節流程圖亦是程式if／else if順序。
...
第9節時間圖把命令與結果分開；恢復成功只支持這次後續能讀，不是永久修復保證。
...
| DHT實物 | 第5節核准表、正背與接線照片、母對母DATA路徑、供電檢查 | 功能起終點清楚，無猜腳序；未核准保留待驗 |
