...
下節預設SSD1306，另提供SSD1315相容性設定；不能直接把教師的選項當成全班答案。
...
完整範例提供128×64、I2C的兩種驅動設定，Week 6與7使用相同選項：

| 實物資料與固定畫面證據 | `OLED_CONTROLLER` |
|---|---:|
| 已核准SSD1306模組 | 1306（預設） |
| BOARD-T01所記錄的SSD1315相容性配置 | 1315 |

在程式頂端將`#define OLED_CONTROLLER 1306`改為核准值，重新Verify。
程式會依這個選項選用對應的U8g2建構子，不必再手動改類別名稱。
只有供電、邏輯電壓、功能腳、解析度與顯示測試都符合，才能把OLED旗標改true。
`OLED_ADDRESS_7BIT`另外填掃描與資料確認的位址；0x3C不是控制器型號。
1315只是教師這顆的相容性紀錄，不是已鑑定晶片。固定文字、框線與數字仍須目視完整、無偏移，
再進倒數整合；SH1106、SPI或其他型號不在這兩個選項內，不以改位址或亂換供電處理。
