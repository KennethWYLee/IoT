...
準備期限：原有零件（下表OLED以外）、USB資料線與電表於Week 2前備妥。
OLED須於核准規格公布後、Week 5前備妥。
電池種類及安全端子於核准後、Week 6前備妥；未公布規格不要猜型號購買。

本表是全班共同實驗的正式材料清單。每人必備零件都在Week 2～7有共同操作，
不是為可能選做的專題預先購買。每位學生保管自己的一套材料；
...
| 品項 | 必須符合的規格 | 每人數量 | 每人參考金額 | Week 2～7共同使用任務 |
|---|---|---:|---:|---|
| ESP32-S3開發板（development board） | N16R8、已焊向下44腳排針；依教師核准的確切電路板（PCB）版型。YD板與原廠DevKitC-1不可直接互套接線圖 | 1片 | NT$390 | Week 2編譯、上傳、序列監控與按鈕輸入；Week 3～7讀取感測器、控制輸出與整合遊戲 |
...
| 溫濕度模組（temperature and humidity module） | YS-31 DHT11 | 1個 | NT$25 | Week 4溫濕度讀取、取樣、失敗與恢復，再與光敏及蜂鳴器整合；不是只為未來專題選配 |
| 三色發光模組（RGB LED） | KY-016類；HW-479須核對限流、共同端與有效準位 | 1個 | NT$9 | Week 5各色輸出與狀態燈；Week 7綠燈允許遮光、紅燈禁止遮光的規則提示 |
| 蜂鳴器模組（buzzer） | KY-012類；HW-508須核對有源型式、供電、腳位、控制準位與驅動電流 | 1個 | NT$14 | Week 4新遮光事件的一次短聲與自動關聲；Week 7限時失敗提示 |
| 小型舵機（servo） | SG90位置型；允許電壓、脈寬與安全角度須驗證，不要求轉滿180度 | 1個 | NT$35 | Week 6受限角度、指針定位與安全停止；Week 7指示有效次數 |
| 電池盒（battery holder） | 4AA、帶開關、帶線；使用已核准安全轉接 | 1個 | NT$15 | Week 6舵機外部供電、極性與共地檢查；Week 7重用同一供電方案 |
| 有機發光顯示器（OLED） | 小型I2C模組；控制器、解析度、位址、腳序及3.3V邏輯／供電相容性待核准 | 1個 | NT$65（歷史單價） | Week 5畫面、倒數與次數；Week 6指針同步顯示；Week 7倒數、次數、規則與結果 |
...
計算：390＋22＋75＋60＋4＋10＋25＋9＋14＋35＋15＋65＝724。
三種杜邦線是NT$25×3＝75；兩顆按鈕是NT$2×2＝4，上表已乘過數量，不要再乘一次。

這些金額沿用[2026年採購紀錄](../docs/hardware/purchased_inventory.md)，只供預算估算，
不是目前賣場報價。OLED既有訂單的歷史單價為NT$65；納入預算不等於型號或接線已核准。
型號核准前不要只看外觀下單，不另加七段顯示器或OLED專用支架。

**NT$724不是全部上課成本。** 尚未包含筆電與充電器、USB資料線、收納容器、
AA電池、安全轉接端子、電表與運費；已有合格用品不必重買。
...
| AA電池（AA battery） | 4顆；化學種類、規格與供電方案須核准，不混用新舊電池 | 1組 | 核准後、Week 6前 | Week 6～7配合電池盒供應舵機電力，不由GPIO供電 |
| 電池盒安全轉接端子（terminal connector） | 型號由教師完成實機驗證後公布；公布前不要購買或自行加工 | 1組 | 核准後、Week 6前 | Week 6～7固定電池盒與舵機供電連接，避免裸線鬆脫或互碰 |
